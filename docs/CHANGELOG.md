...
* **Search**: a regular expression is rebuilt token by token before it is compiled - quoted literals, allow-listed classes, a fixed vocabulary of operators and escapes (length limit, repeat counts up to 100, no repeated groups, back references, look-arounds or named groups; 400 with the reason) - and the search runs under a time budget.
