...
| Regular expression injection (*1.3.1*) | The search API compiled a user-typed regular expression as given (CodeQL `java/regex-injection`) | `Searcher.sanitizeRegex`: at most 200 characters, no quantifier on a group, no back references, syntax checked (400 otherwise); the whole search runs under a 5 s budget (`TimedText`) | done, tested |
