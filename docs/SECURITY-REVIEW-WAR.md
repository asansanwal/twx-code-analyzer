...
| Regular expression injection (*1.3.1*) | The search API compiled a user-typed regular expression as given (CodeQL `java/regex-injection`) | `Searcher.sanitizeRegex` rebuilds the expression from a supported grammar (literals, classes, `. * + ? {n,m} | ^ $`, `\w \s \d \b` and escaped punctuation, plain / non-capturing groups, `(?i)`): at most 200 characters, no quantifier on a group, no back references, look-arounds or named groups, balanced, syntax checked (400 with the reason otherwise); the whole search runs under a 5 s budget (`TimedText`) | done, tested |
