...
public class AnalyzerServlet extends HttpServlet {
    ...
    @Override public void init() throws ServletException {
        ...
        if (flag("enterprise", "tca.enterprise", "TCA_ENTERPRISE", true)) {
            ...
            c.auth = text("auth", "tca.auth", "TCA_AUTH", "builtin").toLowerCase(java.util.Locale.ROOT); c.userHeader = text("userHeader", "tca.userHeader", "TCA_USER_HEADER", ""); c.groupsHeader = text("groupsHeader", "tca.groupsHeader", "TCA_GROUPS_HEADER", ""); c.adminRole = text("adminRole", "tca.adminRole", "TCA_ADMIN_ROLE", "tca-admin"); c.adminGroup = text("adminGroup", "tca.adminGroup", "TCA_ADMIN_GROUP", "");
            for (String a : text("admins", "tca.admins", "TCA_ADMINS", "").split("[,;\\s]+")) if (!a.isEmpty()) c.admins.add(a.toLowerCase(java.util.Locale.ROOT)); String af = text("adminsFile", "tca.adminsFile", "TCA_ADMINS_FILE", ""); if (!af.isEmpty()) c.adminsFile = new File(af);
            ...
            for (String h : text("outboundHosts", "tca.outboundHosts", "TCA_OUTBOUND_HOSTS", "").toLowerCase(java.util.Locale.ROOT).split("[,;\\s]+")) if (!h.isEmpty()) c.outboundHosts.add(h); c.outboundPrivate = flag("outboundPrivate", "tca.outboundPrivate", "TCA_OUTBOUND_PRIVATE", !c.demo); c.designerUrl = text("designerUrl", "tca.designerUrl", "TCA_DESIGNER_URL", ""); c.baseUrl = text("baseUrl", "tca.baseUrl", "TCA_BASE_URL", ""); c.title = text("title", "tca.title", "TCA_TITLE", "TWX Code Analyzer");
    ...
    @Override protected void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        if (p == null || p.isEmpty()) { if (!req.getRequestURI().endsWith("/")) { res.sendRedirect(req.getContextPath() + "/"); return; } p = "/"; }   // the UI uses relative URLs: the root needs its trailing slash (the servlet is mapped on /*, so the root is the context path; nothing of the request URI is echoed)
        Request r = new Request(req, res, p); res.setHeader("X-Frame-Options", "SAMEORIGIN"); res.setHeader("Content-Security-Policy", Api.CSP); res.setHeader("X-Content-Type-Options", "nosniff"); res.setHeader("Referrer-Policy", "same-origin");
        if (api.handle(r)) return;
        if (p.endsWith("/")) p += "index.html";   // the root and the help / swagger folders
