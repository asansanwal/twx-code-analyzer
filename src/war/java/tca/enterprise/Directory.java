...
public class Directory {
    ...
    public static String safeId(String s) { if (s == null) return ""; String id = s.trim().toLowerCase(java.util.Locale.ROOT).replaceAll("[^a-z0-9._-]+", "-").replaceAll("^-+|-+$", ""); return id.length() > 64 ? id.substring(0, 64) : id; }
    ...
    static synchronized Map<String, Object> saveConnectionIn(File file, Map<String, Object> c) throws IOException {
        ...
        String type = str(c, "type").equalsIgnoreCase("studio") ? "studio" : "pc", auth = str(c, "auth").toLowerCase(java.util.Locale.ROOT); if (type.equals("studio") && !auth.equals("bearer") && !auth.equals("basic") && !auth.equals("zen-apikey")) auth = "zen"; if (type.equals("pc")) auth = "basic";
