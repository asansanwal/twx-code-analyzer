...
public class EnterpriseApi extends Api {
    ...
    public Ws workspace(Http x) {
        ...
        if (!who.anonymous()) {
            if (scope != null && scope.startsWith("team:")) { Map<String, Object> t = dir.team(scope.substring(5)); if (t != null && (who.admin || Directory.isMember(t, who.user, who.groups))) { w.kind = "team"; w.id = Directory.str(t, "id"); w.label = Directory.str(t, "name"); w.team = t; w.manage = who.admin || Directory.isTeamAdmin(t, who.user); w.store = storeOf("teams", w.id); } }
            if (w.store == null) { w.kind = "personal"; w.id = wsKey(who.user); w.label = who.name.isEmpty() ? who.user : who.name; w.manage = true; w.store = storeOf("users", w.id); }
        } else {
    ...
    class Repo {
        ...
        String authorization() throws IOException { if (authorization == null) authorization = type.equals("studio") ? Studio.authorization(url, auth.equals("zen-apikey") ? "zen" : auth, user, password, auth.equals("zen-apikey") ? apiKey : "", token, certificate) : "Basic " + Base64.getEncoder().encodeToString((user + ":" + password).getBytes(StandardCharsets.UTF_8)); return authorization; }
        List<Map<String, Object>> apps() throws IOException { try { return type.equals("studio") ? Studio.apps(url, contextRoot, authorization(), certificate) : ProcessCenterClient.apps(url, user, password, certificate); } catch (javax.net.ssl.SSLException e) { throw tls(e); } }
        void export(String snapshotId, File target) throws IOException { try { if (type.equals("studio")) Studio.export(url, contextRoot, authorization(), snapshotId, target, certificate); else tca.baw.ProcessCenterExport.export(url, user, password, snapshotId, target, certificate); } catch (javax.net.ssl.SSLException e) { throw tls(e); } }
        /** A failed TLS handshake, explained: the certificate is not trusted (self-signed servers need their certificate in the definition or in the JVM trust store); never bypassed. */
        ApiException tls(javax.net.ssl.SSLException e) { return new ApiException(502, "TLS handshake with " + url + " failed: " + e.getMessage() + (certificate.isEmpty() ? " - the server certificate is not trusted by the JVM; paste the server certificate (PEM) into the definition or import it into the JVM trust store" : " - the pinned server certificate does not match the certificate the server presents")); }
    ...
    void enterprise(Http x, Store store, Ws w, Identity who, String op, String method, Map<String, String> q) throws Exception {
        ...
        if (op.equals("teams") || op.startsWith("teams/")) {
            ...
            if (method.equals("DELETE")) { boolean d = dir.deleteTeam(tid); if (Boolean.TRUE.equals(body.get("purge")) || "1".equals(q.get("purge"))) { Store ts = storeOf("teams", tid); for (Map<String, Object> m : ts.list()) ts.delete(Directory.str(m, "id")); deleteTree(ts.root); } audit(x, "team.delete", Json.obj("team", tid)); json(x, 200, Json.obj("deleted", d)); return; }
        }
