...
public class Store {
    ...
    /** Report id: timestamp plus a random suffix (not a secret - ids are scoped to their workspace - but unpredictable all the same). */
    public static String newId() { return new java.text.SimpleDateFormat("yyyyMMdd-HHmmss").format(new Date()) + "-" + Integer.toHexString(RANDOM.nextInt(0xffff)); }
    public File dir(String id) { if (id == null || !id.matches("[A-Za-z0-9\\-_]{1,64}")) throw new IllegalArgumentException("bad id"); return SafePath.child(root, id); }
