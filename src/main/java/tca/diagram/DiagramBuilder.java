...
public final class DiagramBuilder {
    ...
    public static Map<String, Object> service(RuleContext c, TwxObject o) {
        ...
        for (ServiceModel.Link k : s.links) edges.add(Json.obj("id", k.id, "from", k.fromItemId, "to", k.toItemId, "name", k.name, "condition", k.condition, "endState", k.endStateId, "error", k.name.toLowerCase(java.util.Locale.ROOT).contains("error")));
